...
#create view notifList as (select usn, date from studgroup s, group_discussion g where s.groupno = g.group_id); )
...
@app.route('/adddatetime', methods = ['GET', 'POST'])
def adddatetime():
    if 'usn' and 'no' in session:
        if request.method == 'POST':
...
def groupdesc():
    if 'usn' in session :
        ...
        if request.method == 'POST':
            ...
            if session['usn'] == lead:
                session['no'] = no
                #cur.execute('insert into group_discussion(group_id) value(%s)', (no))
    ...
    if 'usn' in session: 
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                messageBody ="""
            Thank you for registering! Here are the details of the group_discussion - 
